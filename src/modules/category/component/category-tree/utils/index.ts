...
/**
 * Deep clone a category tree.
 */
export const cloneTree = (items: ICategory[]): ICategory[] =>
  items.map((item) => ({
    ...item,
...
export const findItemContext = (
  items: ICategory[],
  id: number,
): { parent: ICategory[]; index: number } | null => {
 ...
 */
export const collectDescendantIds = (item: ICategory): Set<number> => {
  const ids = new Set<number>();
  const walk = (children: ICategory[]) => {
