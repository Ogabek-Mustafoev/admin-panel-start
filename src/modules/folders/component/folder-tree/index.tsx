...
import type { FolderTreeProps, DropPosition, ITreeHistoryEntry } from "./types";
...
export const FolderTree: FC<FolderTreeProps> = ({
  ...
  const [treeData, setTreeData] = useState<IFolder[]>([]);
  ...
  // DnD state
  const [activeItem, setActiveItem] = useState<IFolder | null>(null);
  ...
  useEffect(() => {
    if (foldersData) {
      setTreeData(cloneTree(foldersData));
    }
  }, [isFetching]);
  ...
  const renderItems = (items: IFolder[], depth = 0) =>
    items.map(item => {
          ...
          {activeItem && (
            <div className="bg-themeBge blur-bg flex items-center gap-2 rounded-lg px-3 py-2 opacity-95 shadow-xl">
              ...
              <span className="text-sm font-medium">
                 {activeItem?.name?.[locale] || activeItem?.name?.cn}
              </span>
            </div>
          )}
        </DragOverlay>
